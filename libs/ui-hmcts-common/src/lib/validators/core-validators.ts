...
export class CoreValidators{
  ...
  public static postcodeValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null  => {
      //TODO this isn't working correctly -- investigate later
      // for the time being use a default pattern matcher
      console.log('Postcode validator');
      if (control.value === null || control.value === '')  return { postcode: true }
